...
import { getDetailReturnBycode } from "../../apis/return-product-module"
...
function ReturnProductDetail() {
  ...
  useQueries([
    {
      queryKey: ["getDetailReturnImport", returnCode],
      queryFn: async () => {
        const response = await getDetailReturnBycode(returnCode)
  ...
  const handleClickImportReturnGood = async (event) => {
    ...
    await approveImportMutation.mutate(detailReturnImport?.returnsId)
  }
